...
export async function run(data: { teamId: string; zipPath: string; botCount?: number }) {
  ...
  try {
    ...
    const { latencies: loadLatencies, tps, stability, tradeStats: loadTradeStats } = await fleet.run(data.botCount ?? 50, 100);
    console.log(`[runner] Load test done: ${loadLatencies.length} acks, ${tps} TPS, stability=${(stability * 100).toFixed(1)}%`);

    redis.xadd('events', '*', 'data', JSON.stringify({
      ts: Date.now(), runId, teamId: data.teamId, level: 'info', event: 'load_done',
      tps, stability, tradeStats: loadTradeStats,
    })).catch(() => {});
    ...
    console.log(`[runner] Run ${runId} complete: score=${score} p99=${metrics.p99}ms tps=${tps} stability=${(stability * 100).toFixed(1)}%`);
  } catch (err) {
